...
class _TrackingWidgetState extends StateMVC<TrackingWidget>
  ...
  Future<void> getDriverDetails(String orderId) async {
    String url = '${GlobalConfiguration().getString('api_base_url')}ordersdriver/$orderId';
    var response = await http.get(url);

    if (response.statusCode == 200) {
      final driverDetails = driverDetailsFromJson(response.body);
      setState(() {
        driverName = driverDetails.data.name;
        diverNo = driverDetails.data.value;
      });
    } else {
  ...
  void initState() {
    ...
    getDriverDetails(widget.routeArgument.id);
    _tabController =
                  ...
                  FlatButton(
                    ...
                    child: Row(
                      ...
                      children: Helper.getStarsList(
                          double.parse(
                              _con.order.productOrders[0].product.market.rate),
                          size: 35),
                    ),
                  ),
                ],
                                              ...
                                              showDialog(
                                                ...
                                                builder:
                                                  ...
                                                  return AlertDialog(
                                                    ...
                                                    content: Text('There shall be no refund upon cancellation. Would you still like to proceed with cancellation?'),
                                                    contentPadding:
